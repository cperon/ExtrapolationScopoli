...
# Load data
  ...
  load('results/GAM_models/2012/foraging_v2/track_oceano_2012-cleaned.Rdata')
  ALL2012 <- track_oceano
  #ALL2011 <- ALL
  ALL2012$Site <- as.character(ALL2012$Site)    
  ALL2012$Site <- ifelse(ALL2012$Site=='Frioul', 'Riou', ALL2012$Site)      
  ALL2012$Site <- as.factor(ALL2012$Site)  
  table(ALL2012$Site)

# Loop on site

shrw_2012 <- NULL
for(site in levels(ALL2012$Site)){
  print(site)
  
  X <- ALL2012[ALL2012$Site %in% site,]
  X1 <- X
  ...
  X3 <- X2[X2$Dup==FALSE,]
  shrw_2012 <- rbind(shrw_2012, X3)
}

shrw_2012$Dup <- NULL
shrw_2012$rownames <- NULL

save(shrw_2012, file='results/GAM_models/2011/RawLocSim/_v2/extrapolation_analysis/v_2/ExtrapolationScopoli/data/shrw_2012_nodup.Rdata')
